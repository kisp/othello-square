...
(defun compile-application (files output &key shebang)
  (with-compilation-environment
      (with-open-file (out output :direction :output :if-exists :supersede)
        ...
        (format out "import jscl from \"./jscl.js\"~%")
        (format out "import m from \"mithril\"~%")
        (format out "var values = jscl.internals.pv;")
        (format out "var internals = jscl.internals;")
        (dolist (input files)
        ...
        (format out "export default {
exports_for_js: internals.intern(\"EXPORTS-FOR-JS\", \"MYAPP\").fvalue
}"))))
